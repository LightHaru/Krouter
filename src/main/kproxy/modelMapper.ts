...
const MODEL_PATTERNS: Record<string, { match: RegExp; alias: string }[]> = {
  kiro: [
    { match: /opus/i, alias: 'claude-opus-4.8' },
    { match: /sonnet/i, alias: 'claude-sonnet-4.5' },
    { match: /haiku/i, alias: 'claude-haiku-4.5' }
  ],
  antigravity: [
    {
...
const DEFAULT_MAPPINGS: ModelMapping[] = [
  ...
  { ideModel: 'claude-opus-4.8', krouterModel: 'claude-opus-4.8', ideType: 'kiro', enabled: true },
  {
    ideModel: 'claude-sonnet-4.5',
    krouterModel: 'claude-sonnet-4.5',
    ideType: 'kiro',
    enabled: true
  },
  {
    ideModel: 'claude-haiku-4.5',
    krouterModel: 'claude-haiku-4.5',
    ideType: 'kiro',
    enabled: true
  },
  {
    ideModel: 'claude-sonnet-4-6',
    krouterModel: 'claude-sonnet-4-6',
    ideType: 'kiro',
    enabled: true
  },
  { ideModel: 'claude-opus-4-6', krouterModel: 'claude-opus-4-6', ideType: 'kiro', enabled: true },
  {
    ideModel: 'anthropic.claude-sonnet-4-6-v1',
    krouterModel: 'claude-sonnet-4-6',
    ideType: 'kiro',
    enabled: true
  },
  {
    ideModel: 'anthropic.claude-opus-4-6-v1',
    krouterModel: 'claude-opus-4-6',
    ideType: 'kiro',
    enabled: true
  },
  {
    ideModel: 'us.anthropic.claude-sonnet-4-6-v1:0',
    krouterModel: 'claude-sonnet-4-6',
    ideType: 'kiro',
    enabled: true
  },
  {
    ideModel: 'us.anthropic.claude-opus-4-6-v1:0',
    krouterModel: 'claude-opus-4-6',
    ideType: 'kiro',
    enabled: true
  },
  ...
  {
    ideModel: 'claude-3.5-sonnet',
    krouterModel: 'claude-sonnet-4-6',
    ideType: 'copilot',
    enabled: true
  },
  {
    ideModel: 'claude-sonnet-4',
    krouterModel: 'claude-sonnet-4-6',
    ideType: 'copilot',
    enabled: true
  },
  ...
  {
    ideModel: 'gemini-3-flash-agent',
    krouterModel: 'claude-sonnet-4-6',
    ideType: 'antigravity',
    enabled: true
  },
  {
    ideModel: 'gemini-3.5-flash-low',
    krouterModel: 'claude-sonnet-4-6',
    ideType: 'antigravity',
    enabled: true
  },
  {
    ideModel: 'gemini-3.5-flash-extra-low',
    krouterModel: 'claude-sonnet-4-6',
    ideType: 'antigravity',
    enabled: true
  },
  {
    ideModel: 'gemini-pro-agent',
    krouterModel: 'claude-opus-4-6',
    ideType: 'antigravity',
    enabled: true
  },
  {
    ideModel: 'gemini-3.1-pro-low',
    krouterModel: 'claude-sonnet-4-6',
    ideType: 'antigravity',
    enabled: true
  }
...
export class ModelMapper {
    ...
    try {
      if (fs.existsSync(this.configPath)) {
        ...
        if (Array.isArray(data.mappings)) {
          const persisted = data.mappings as ModelMapping[]
          const keys = new Set(persisted.map((mapping) => `${mapping.ideType}:${mapping.ideModel}`))
          return [
            ...persisted,
            ...DEFAULT_MAPPINGS.filter(
              (mapping) => !keys.has(`${mapping.ideType}:${mapping.ideModel}`)
            )
          ]
        }
      }
  ...
  mapModel(ideModel: string, ideType?: IdeType): string | null {
    ...
    const exactMatch = this.mappings.find(
      (m) => m.enabled && m.ideModel === lookup && (!ideType || m.ideType === ideType)
    )
    if (exactMatch) return exactMatch.krouterModel

    // Prefix match
    const prefixMatch = this.mappings.find(
      (m) =>
        m.enabled &&
        (!ideType || m.ideType === ideType) &&
    ...
    if (ideType) {
      ...
      for (const { match, alias } of patterns) {
        if (match.test(lookup)) {
          const aliasMapping = this.mappings.find(
            (m) => m.enabled && m.ideModel === alias && m.ideType === ideType
          )
  ...
  getDefaultTarget(ideType: IdeType): string | null {
    return (
      this.mappings.find((mapping) => mapping.enabled && mapping.ideType === ideType)
        ?.krouterModel || null
    )
  }
